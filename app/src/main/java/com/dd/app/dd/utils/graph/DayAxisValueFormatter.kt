...
class DayAxisValueFormatter(chartData: BarLineChartBase<*>) : ValueFormatter() {

    private val mMonths = arrayOf(
        "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
    )

    private val chart: BarLineChartBase<*> = chartData
    ...
    override fun getFormattedValue(value: Float): String? {
        ...
        return if (chart.visibleXRange > 30 * 6) {
            ...
            val text = if (dayOfMonth == 0) "" else "$dayOfMonth$appendix $monthName"
            //UiUtils.log("Time", "--> $text")
            if (dayOfMonth == 0) "" else dayOfMonth.toString() + "" /*+ appendix + " " + monthName*/
        }
    }
